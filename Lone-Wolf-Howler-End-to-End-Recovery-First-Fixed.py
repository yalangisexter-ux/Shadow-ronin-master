#!/usr/bin/env python3
"""
Lone-Wolf-Howler — Recovery-First, Restart-Aware, Self-Healing Pipeline
=======================================================================
Fixes 22.log:
...
HF_REPO = os.environ.get("HF_REPO", "Lone-Wolf-Howler")
HF_REPO_ID = f"{HF_USER}/{HF_REPO}"

RUNTIME_HF_REPO = os.environ.get("RUNTIME_HF_REPO", "Lone-Wolf-Howler-Runtime")
...
STATE_BASE_DIR = os.environ.get("STATE_BASE_DIR", os.path.join(KAGGLE_WORKING_DIR, "lone_wolf_howler_state"))
...
_TMP_CANDIDATES = [
    os.environ.get("LONE_WOLF_HOWLER_SCRATCH", "/kaggle/tmp/lone_wolf_howler_scratch"),
    "/kaggle/tmp/lone_wolf_howler_scratch",
    "/tmp/lone_wolf_howler_scratch",
...
def log(msg: str) -> None:
    with _LOG_LOCK:
        print(f"[Lone-Wolf-Howler] {msg}", flush=True)
...
def check_hf_recovery() -> Dict[str, Any]:
    ...
    fp16_gguf = any(
        f in files for f in (
            "fp16_model/gguf_output/lone-wolf-howler-f16.gguf",
            "gguf/lone-wolf-howler-f16.gguf",
        )
    )
...
def recover_gguf_stage_state(published_gguf_names: List[str]) -> None:
    state = _load_gguf_state()
    mapping = {f"lone-wolf-howler-{q.lower()}.gguf": q for q in QUANT_TARGETS}
    mapping["lone-wolf-howler-f16.gguf"] = "F16"
    updated = 0
...
def _write_cuda_driver_cmake_workaround(path: str, libcuda_path: str) -> None:
    ...
    content = f"""# Auto-generated by Lone-Wolf-Howler — CUDA::cuda_driver workaround
if(NOT TARGET CUDA::cuda_driver)
  if(NOT EXISTS "{safe}")
    message(FATAL_ERROR
      "Lone-Wolf-Howler CUDA workaround: libcuda not found at {safe}")
  endif()
  ...
  message(STATUS
    "Lone-Wolf-Howler: defined CUDA::cuda_driver -> {safe}")
endif()
"""
...
def _prune_llama_build_objects(build_dir: str) -> None:
    ...
    for entry in os.listdir(build_dir):
        if entry in ("bin", "CMakeCache.txt", "lone_wolf_howler_cuda_driver.cmake",
...
def ensure_llama_cpp() -> str:
    ...
    if cuda_requested:
        ...
        workaround = os.path.join(build_dir, "lone_wolf_howler_cuda_driver.cmake")
...
def _ensure_f16_gguf(fp16_dir: str, converter: str) -> str:
    ...
    f16 = os.path.join(GGUF_OUTPUT_DIR, "lone-wolf-howler-f16.gguf")
...
def _publish_gguf_hf(path: str) -> None:
    ...
    if fname == "lone-wolf-howler-f16.gguf":
...
def export_and_quantize_gguf(fp16_dir: str) -> List[str]:
    ...
    os.makedirs(GGUF_OUTPUT_DIR, exist_ok=True)
    f16 = os.path.join(GGUF_OUTPUT_DIR, "lone-wolf-howler-f16.gguf")
    f16_name = "lone-wolf-howler-f16.gguf"
    ...
    for quant in QUANT_TARGETS:
        out = os.path.join(GGUF_OUTPUT_DIR, f"lone-wolf-howler-{quant.lower()}.gguf")
...
def verify_remote_release_exact() -> None:
    ...
    for q in QUANT_TARGETS:
        required.append(f"gguf/lone-wolf-howler-{q.lower()}.gguf")
...
def run_recovery_mode(recovery: Dict[str, Any], start_time: float) -> int:
    ...
    # Final local cleanup
    f16_src = os.path.join(GGUF_OUTPUT_DIR, "lone-wolf-howler-f16.gguf")
...
def _repair_bitsandbytes_for_stack() -> None:
    ...
    try:
        run_cmd([sys.executable, "-m", "pip", "cache", "purge"],
                check=False)
    except Exception:
        pass
...
def _repair_json_text(raw: str) -> Tuple[Optional[Any], Optional[str], Optional[str]]:
    text = raw.strip().lstrip("\ufeff")
    if not text:
        return None, None, "empty record"
    candidates = [("strict_json", text)]
    if text.startswith("```") and text.endswith("```"):
        lines = text.splitlines()
        if len(lines) >= 3:
            candidates.append(("markdown_fence_removed", "\n".join(lines[1:-1]).strip()))
    no_trailing = re.sub(r",(\s*[}\]])", r"\1", text)
    if no_trailing != text:
        candidates.append(("trailing_commas_removed", no_trailing))
    candidates.append(("python_literal", text))
    if no_trailing != text:
        candidates.append(("python_literal_trailing_commas_removed", no_trailing))
    seen = set()
    for method, candidate in candidates:
        ...
        seen.add(candidate)
        try:
            return json.loads(candidate), (None if method == "strict_json" else method), None
        except Exception:
            pass
        if method.startswith("python_literal"):
            try:
                return ast.literal_eval(candidate), method, None
            except Exception:
                pass
    return None, None, "invalid JSON/JSON-like record"
...
def iter_jsonl_records(path: str) -> Iterable[Tuple[int, Any, Optional[str]]]:
    with open(path, "r", encoding="utf-8", errors="replace") as fh:
        for line_number, raw in enumerate(fh, 1):
            if not raw.strip():
                continue
            obj, repair, error = _repair_json_text(raw)
            if error:
                yield line_number, None, error
            else:
                yield line_number, obj, repair
...
def normalize_jsonl_file(source_path: str, output_path: str) -> Dict[str, Any]:
    valid, repaired, invalid = 0, 0, 0
    invalid_lines, repair_details = [], []
    with open(output_path, "w", encoding="utf-8") as out:
        for line_no, obj, status in iter_jsonl_records(source_path):
            if status and status.startswith("invalid"):
                invalid += 1
                invalid_lines.append({"line": line_no, "error": status})
                continue
            clean = obj if isinstance(obj, dict) else ({"data": obj} if isinstance(obj, list) else {"text": str(obj)})
            if status:
                repaired += 1
                repair_details.append({"line": line_no, "repair": status})
            out.write(json.dumps(clean, ensure_ascii=False) + "\n")
            valid += 1
    return {"source": os.path.abspath(source_path), "normalized": os.path.abspath(output_path),
            "valid_records": valid, "repaired_records": repaired, "invalid_lines": invalid,
            "invalid_details": invalid_lines[:1000], "repair_details": repair_details[:1000],
...
def validate_and_normalize_jsonl_files(
    ...
    report = {"generated_at": time.strftime("%Y-%m-%dT%H:%M:%S"),
              ...
              "total_valid_records": 0, "total_repaired_records": 0,
    ...
    for source in data_files:
        ...
        report["total_repaired_records"] += result["repaired_records"]
    ...
    log(f"JSONL validation: {report['total_valid_records']} valid, "
        f"{report['total_repaired_records']} repaired, "
...
def combine_normalized_jsonl_files(normalized_files: List[str]) -> Tuple[str, Dict[str, Any]]:
    os.makedirs(PERMANENT_DATASET_DIR, exist_ok=True)
    combined_path = os.path.join(PERMANENT_DATASET_DIR, "combined_training.jsonl")
    meta_path = os.path.join(PERMANENT_DATASET_DIR, "combined_training_meta.json")
    source_hashes = {os.path.abspath(x): _sha256_file(x) for x in sorted(normalized_files)}
    expected = {"version": 2, "model_id": MODEL_ID, "source_files": source_hashes}
    if os.path.isfile(combined_path) and os.path.isfile(meta_path):
        try:
            old=json.loads(Path(meta_path).read_text(encoding="utf-8"))
            if all(old.get(k)==v for k,v in expected.items()):
                return combined_path, old
        except Exception: pass
    seen=set(); total=0; records_seen=0; duplicates_removed=0; per_file=[]
    with open(combined_path,"w",encoding="utf-8") as out:
        for source in sorted(normalized_files):
            count=0
            with open(source,"r",encoding="utf-8",errors="replace") as fh:
                for line_no,raw in enumerate(fh,1):
                    ...
                    key=hashlib.sha256(json.dumps(record,ensure_ascii=False,sort_keys=True,separators=(",",":" )).encode()).hexdigest()
                    if key in seen:
                        duplicates_removed += 1
                        continue
                    seen.add(key)
                    record["_source_file"]=os.path.basename(source); record["_source_line"]=line_no
                    out.write(json.dumps(record,ensure_ascii=False)+"\n"); count+=1; total+=1
            per_file.append({"source":os.path.abspath(source),"records_added":count})
    meta={**expected,"total_records":total,"duplicates_removed":sum(x["records_added"] for x in per_file)-total,"per_file":per_file,"sha256":_sha256_file(combined_path),"generated_at":time.strftime("%Y-%m-%dT%H:%M:%S")}
    Path(meta_path).write_text(json.dumps(meta,indent=2,ensure_ascii=False),encoding="utf-8")
    if not total: raise RuntimeError("Combined JSONL contains zero records.")
    log(f"COMBINED TRAINING JSONL: {total} unique records from {len(normalized_files)} files")
    return combined_path,meta


def get_or_create_dataset(tokenizer, normalized_files: List[str]):
    from datasets import load_dataset
    combined_path, combined_meta = combine_normalized_jsonl_files(normalized_files)
    ...
    expected={"version":2,"model_id":MODEL_ID,"combined_sha256":combined_meta["sha256"],"total_records":combined_meta["total_records"]}
    if os.path.exists(cache_meta) and os.path.exists(cache_path):
        try:
            if json.loads(Path(cache_meta).read_text(encoding="utf-8"))==expected:
                log("Using validated combined training dataset cache.")
                return load_dataset("json",data_files=cache_path,split="train")
        except Exception as exc: log(f"Training cache invalid; rebuilding: {exc}")
    rows=[]
    with open(combined_path,"r",encoding="utf-8",errors="replace") as fh:
        for line_no,raw in enumerate(fh,1):
            if not raw.strip(): continue
            try: record=json.loads(raw)
            except Exception as exc: log(f"WARNING: combined JSONL line {line_no} skipped: {exc}"); continue
            if not isinstance(record,dict): record={"text":str(record)}
            rows.append(standardize_to_chat(record,tokenizer))
    if not rows: raise RuntimeError("No usable training examples were produced.")
    with open(cache_path,"w",encoding="utf-8") as fh:
        for row in rows: fh.write(json.dumps(row,ensure_ascii=False)+"\n")
    Path(cache_meta).write_text(json.dumps({**expected,"prepared_examples":len(rows)},indent=2,ensure_ascii=False),encoding="utf-8")
    log(f"Prepared {len(rows)} combined training examples.")
    return load_dataset("json",data_files=cache_path,split="train")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with open(r, "w", encoding="utf-8") as fh:
            fh.write(
                ...
                f"# Lone-Wolf-Howler\n\n"
                f"Validated Lone-Wolf-Howler release.\n\n"
                f"## Contents\n\n"
...
def _capture_runtime_bundle() -> Optional[str]:
    ...
    try:
        ...
        Path(os.path.join(bundle, "README.md")).write_text(
            "# Lone-Wolf-Howler Runtime Bundle\\n\\n"
...
def run_scratch_mode(start_time: float, bnb_preflight_done: bool = False) -> int:
    log("=" * 72)
    log("LONE-WOLF-HOWLER MASTER DEVELOPMENT PIPELINE")
...
def main() -> int:
    start = time.time()
    log("=" * 72)
    log("Lone-Wolf-Howler — Recovery-First, Restart-Aware Pipeline TEST")
    log("=" * 72)
...
if __name__ == "__main__":
    ...
    print(f"[Lone-Wolf-Howler] exit_code={_code}", flush=True)